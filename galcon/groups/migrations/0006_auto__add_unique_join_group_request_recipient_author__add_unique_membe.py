...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Join_Group_Request', fields ['recipient', 'author']
        db.create_unique('groups_join_group_request', ['recipient_id', 'author_id'])

        # Adding unique constraint on 'Membership', fields ['player', 'group']
        db.create_unique('groups_membership', ['player_id', 'group_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Membership', fields ['player', 'group']
        db.delete_unique('groups_membership', ['player_id', 'group_id'])

        # Removing unique constraint on 'Join_Group_Request', fields ['recipient', 'author']
        db.delete_unique('groups_join_group_request', ['recipient_id', 'author_id'])
    ...
    models = {
        ...
        'groups.join_group_request': {
            'Meta': {'unique_together': "(('author', 'recipient'),)", 'object_name': 'Join_Group_Request'},
        ...
        'groups.membership': {
            'Meta': {'unique_together': "(('player', 'group'),)", 'object_name': 'Membership'},
